...
class PascalVOCDetectionEvaluator(DatasetEvaluator):
    ...
    def evaluate(self):
        ...
        """
            [
                {"image_id": id, "category_id": category, "bbox": bbox, "score": score}
            ]
        """
        ...
        if self.save_detection_result:
            ...
            json.dump(
                tide_result,
                open(self.save_path.format(self._dataset_name), "w")
            )
        ...
        res_file_template = "comp4" + salt + "_det_" + image_set + "_{:s}.txt"
        res_file_template = os.path.join(
            devkit_path, "results", "VOC" + year, "Main", res_file_template
        )
        pp = os.path.join(devkit_path, "results", "VOC" + year, "Main")
        if not os.path.exists(pp):
            os.system(f"mkdir -p {pp}")
