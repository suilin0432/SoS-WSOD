...
def transform_proposals_multi(dataset_dict, image_shape, transforms, *, proposal_topk, min_box_size=0):
    """
    ...
    if "proposal_boxes" in dataset_dict:
        ...
        final_keep = torch.zeros(len(boxes)).bool()

        keep = boxes.unique_boxes()
        final_keep[keep] = True

        # 不进行筛选, 在最后进行筛选所有变换进行结束之后进行筛选
        ...
        dataset_dict["final_keep"] = final_keep
...
def build_augmentation(cfg, is_train):
    ...
    if is_train:
        if not cfg.MODEL.META_ARCHITECTURE == "MultiInputRCNN":
