...
class ImageList(object):
    ...
    def from_tensors(
        ...
        """
        传入参数:
            tensors: Sequence(torch.Tensor), 每一项 shape 为 (Ci, Hi, Wi), 会将所有的 tensor 都使用 pad_value
            进行 padding 到相同的 shape.
            size_divisibility: Some backbones require the input height and width to be divisible by a
                specific integer. This is typically true for encoder / decoder type networks
                with lateral connection (e.g., FPN) for which feature maps need to match
                dimension in the "bottom up" and "top down" paths. Set to 0 if no specific
                input size divisibility is required. ResNet 和 VGG 简单的做 backbone 的时候设置为 0, 即不用进行
                padding -> 已经验证了, 确实是不用的. FPN用是因为有不同 feature map 的融合, 所以需要保证输出的 
                feature map 尺寸是一致的
        Args:
        ...
        # 先是获取到每个图片的 shape, 然后获取shape每个维度的最大值
        max_size = (
        ...
        # 如果要求了 size_divisibility 那么还要将 max size 进行补全
        ...
        # 下面的主要是负责将所有的 image padding 补齐到相同的尺度
        if len(tensors) == 1:
