...
# 使用 能够进行 放缩 refinement 分支 lr 的 optimizer
...
#         # 暂时不考虑 norm layer 的问题...(VGG)中没有
#         if "bn" in key.lower():
#             weight_decay = cfg.SOLVER.WEIGHT_DECAY_NORM
#         elif "refine" in key and "bias" in key:
#             lr = cfg.SOLVER.BASE_LR * cfg.SOLVER.BIAS_LR_FACTOR * cfg.SOLVER.REFINE_LR_SCALE
#             weight_decay = cfg.SOLVER.WEIGHT_DECAY_BIAS
#         elif "refine" in key and "bias" not in key:
#             lr = cfg.SOLVER.BASE_LR * cfg.SOLVER.REFINE_LR_SCALE
#         elif "refine" not in key and "bias" in key:
#             lr = cfg.SOLVER.BASE_LR * cfg.SOLVER.BIAS_LR_FACTOR
#             weight_decay = cfg.SOLVER.WEIGHT_DECAY_BIAS
#         elif "refine" not in key and "bias" not in key:
#             pass
#         else:
#             assert False, "wrong type in building solver"
...
def build_optimizer(cfg: CfgNode, model: torch.nn.Module) -> torch.optim.Optimizer:
    ...
    params: List[Dict[str, Any]] = []
    memo: Set[torch.nn.parameter.Parameter] = set()
    if cfg.SOLVER.REFINE_SCALE_ON:
        print("using higher lr on refinement branch")
        for key, value in dict(model.named_parameters()).items():
            # print(key)
            if not value.requires_grad:
                continue
        ...
        print("using the same lr on refinement branch")
