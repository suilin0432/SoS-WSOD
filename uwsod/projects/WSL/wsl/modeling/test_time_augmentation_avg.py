...
from detectron2.modeling.meta_arch import MultiInputRCNN
...
class GeneralizedRCNNWithTTAAVG(nn.Module):
    ...
    def __init__(self, cfg, model, tta_mapper=None, batch_size=1):
        ...
        if isinstance(model, DistributedDataParallel):
            model = model.module
        assert isinstance(model, GeneralizedRCNNWSL) or isinstance(model, MultiInputRCNN), "TTA is only supported on GeneralizedRCNNWSL. Got a model of type {}".format(type(model))
        self.cfg = cfg.clone()
