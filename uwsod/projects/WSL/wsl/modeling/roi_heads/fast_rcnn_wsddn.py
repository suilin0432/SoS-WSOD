...
def fast_rcnn_inference(boxes, scores, image_shapes, score_thresh, nms_thresh, topk_per_image):
    ...
    # 四项分别对应于: 
    # 1. result 是检测 bbox 信息, 2. filter_inds 是过滤得到的信息, 3. all_scores 是 R * (num_class + 1) 的原始分数, 4. all_boxes 是 R * 4 的原始 boxes 信息
    return (
...
class WSDDNOutputs(object):
    ...
    def bce_loss(self, predicts, targets, reduction):
        # 只是在 半精度的时候使用
        # assert isinstance(predicts, torch.cuda.HalfTensor) and isinstance(targets, torch.cuda.HalfTensor), "Both the predicts and targets should be torch.cuda.HalfTensor!!!"
        # print("type of predicts and targets: ", type(predicts), type(targets), isinstance(predicts, torch.cuda.HalfTensor), isinstance(targets, torch.cuda.HalfTensor))
        if reduction == "mean":
            BCE_loss = -torch.mean(targets*torch.log(predicts) + (1-targets)*torch.log(1-predicts))
        elif reduction == "sum":
            BCE_loss = -torch.sum(targets*torch.log(predicts) + (1-targets)*torch.log(1-predicts))
        return BCE_loss
    ...
    def binary_cross_entropy_loss(self):
        ...
        # if isinstance(self.predict_probs_img(), torch.cuda.HalfTensor):
        if self.amp:
