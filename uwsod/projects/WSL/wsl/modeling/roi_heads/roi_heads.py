...
def get_image_level_gt(targets, num_classes):
    ...
    # 获取每个 image 的 gt_classes label. 用 unique 进行处理 image 中包含的所有 instance 信息
    gt_classes_img = [torch.unique(t.gt_classes, sorted=True) for t in targets]
    # 将 gt_classes_img 中的 label 转化为 int64, 默认好像是 float32
    gt_classes_img_int = [gt.to(torch.int64) for gt in gt_classes_img]
    # gt_classes_img_oh 是获取展开之后的 0, 1 标签,
    # eg: num_classes 是 5, gt 为 [2,3] 时候 对应 image 产生的标签会是 [0,0,1,1,0]
...
class ROIHeads(torch.nn.Module):
    ...
    def label_and_sample_proposals(
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            ...
            # 与 uwsod_head 的不同就只是这里, 这里没有要求 not self.cls_agnostic_bbox_known
            if has_gt:
