...
class StandardRPNHeadWSL(nn.Module):
    ...
    def __init__(self, *, in_channels: int, num_anchors: int, box_dim: int = 4):
        ...
        # 进行 物体 objectness logits 预测的 1x1 conv
        # 1x1 conv for predicting objectness logits, 每个 anchor 只对应于一个是前景还是背景的 位 即可.
        self.objectness_logits = nn.Conv2d(in_channels, num_anchors, kernel_size=1, stride=1)
        # 进行 bbox regression delta 预测的 1x1 conv
    ...
    def forward(self, features: List[torch.Tensor]):
        ...
        pred_anchor_deltas = []
        # 对每一个 feature map 都进行 objectness logits 和 anchor deltas 的预测产生
        for x in features:
...
class RPNWSL(nn.Module):
    ...
    def from_config(cls, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        ret = {
            ...
            # 默认设置为不会筛选掉超出边界的 anchors
            "anchor_boundary_thresh": cfg.MODEL.RPN.BOUNDARY_THRESH,
            "box2box_transform": Box2BoxTransform(weights=cfg.MODEL.RPN.BBOX_REG_WEIGHTS),
            "box_reg_loss_type": cfg.MODEL.RPN.BBOX_REG_LOSS_TYPE,
            "smooth_l1_beta": cfg.MODEL.RPN.SMOOTH_L1_BETA,
            "has_reg": cfg.MODEL.RPN.HAS_REG
        }
        ...
        # RPN.IOU_THRESHOLDS = [0.3, 0.7], 在中间范围的会被忽略掉, 而且传入了 allow_low_quality_matches = True, 表示允许低质量匹配
    ...
    def forward(
        ...
        # 获取提取 rpn 的 feature map
        features = [features[f] for f in self.in_features]
        # mrrp_on 的时候把 feature map 拆开
        if self.mrrp_on:
            # torch.chunk: eg: a (shape:(3, 10, 10)) -> torch.chunk(a, 3) -> tuple(Tensor(1, 10, 10), Tensor(1, 10, 10), Tensor(1, 10, 10))
            features = [torch.chunk(f, self.mrrp_num_branch) for f in features]
            # 因为 chunk 之后产生的是 tuple, 所以要再拆开一次
        ...
        # 这才是 WSLRPN 的代码
            ...
            losses = {}
        # 获取最终的 proposals, 其中主要包含了 proposal_boxes, objectness_logits, level_ids 的相关属性
    ...
    def predict_proposals(
        ...
        # 会把 梯度 detach 掉, 之后就直接传入到 find_top_rpn_proposals_group 中进行处理, 找到对应的高质量 proposals 即可
        pred_objectness_logits = [t.detach() for t in pred_objectness_logits]
        pred_anchor_deltas = [t.detach() for t in pred_anchor_deltas]
        # 首先 anchor generator 产生的 anchors 对所有图片来说都是相同的, 但是对 不同的 image 后面要进行不同的 delta 等处理
        pred_proposals = self._decode_proposals(anchors, pred_anchor_deltas)
