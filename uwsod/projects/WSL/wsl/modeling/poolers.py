...
class ROIPooler(nn.Module):
    ...
    def forward(
        ...
        # print("pooler:", num_level_assignments)
        # print(self.level_poolers)
        # print(len(x), x[0].shape, len(box_lists), box_lists[0].tensor.shape)
        ...
        if num_level_assignments == 1:
            # print("type in pooling", type(x[0]), type(pooler_fmt_boxes), isinstance(x[0], torch.cuda.HalfTensor), isinstance(pooler_fmt_boxes, torch.cuda.HalfTensor))
            if isinstance(x[0], torch.cuda.HalfTensor):
                # return self.level_poolers[0](x[0].float(), pooler_fmt_boxes)
                return self.level_poolers[0](x[0], pooler_fmt_boxes.half())
            else:
                # print(self.level_poolers[0](x[0], pooler_fmt_boxes).shape)
