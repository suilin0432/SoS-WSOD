...
class GeneralizedRCNNWSL(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        需要的主要参数:
            ...
            proposal_generator: wslrpn 或者 只用 proposals
            roi_heads: wsod head 后续网络整体
        """
        """
    ...
    def from_config(cls, cfg):
        # 默认是 build_mrrp_ws_resnet_backbone
    ...
    def forward(self, batched_inputs):
        ...
        # exit()
        # 训练的时候是会有 instances 在 x 中的, 可以提取出来是 Instances 实例
        ...
        # 用 backbone 处理 features
        features = self.backbone(images.tensor)

        # 有 rpn 的时候
        if self.proposal_generator:
            # 使用 proposal generator 进行 proposals 的生成, 第二项的 loss RPNWSL 中的实现是空的, 因此直接没有利用
            proposals, _ = self.proposal_generator(images, features, gt_instances)
            # 对 proposals 中
            # 对应的是不同 image 产生的 proposals
            for i, p in enumerate(proposals):
                # corner case -> 本image没有产生任何的 proposals
                if len(p) == 0:
                    device = images.tensor.device
                    image_size = images.image_sizes[0]
                    # 产生一个占据全局的 anchor... 并且设置 score 为 1 
                    b_new = Boxes(
                        torch.tensor(
                    ...
                    # 构建当前 feature map 的 Instances 类
                    p = Instances(image_size)
                    p.proposal_boxes = b_new
                    p.objectness_logits = s_new
                    p.level_ids = [0]
                    proposals[i] = p
        # 没有 rpn 的时候 直接从 input 中提取所有的 proposals 出来
        else:
            assert "proposals" in batched_inputs[0]
            proposals = [x["proposals"].to(self.device) for x in batched_inputs]
            proposal_losses = {}
        # roi_heads 获取 detector loss
        _, detector_losses = self.roi_heads(images, features, proposals, gt_instances)
        # 如果是有 proposal generator 时, 会获取 rpn 的 loss
        if self.proposal_generator:
            # 这里会从 roi_heads 中取出 targets
    ...
    def preprocess_image(self, batched_inputs):
        ...
        提取出来 image, 并将 image 放在 gpu 上, 进行 normalization 之后
        """
