...
def add_wsl_config(cfg):
    ...
    _C.WSL.MIST_P = 0.10
    _C.WSL.MIST_THRE = 0.05
    _C.WSL.MIST_TYPE = "nms" # "nms" or "wetectron"
    ...
    _C.WSL.SAMPLING = CN()
    _C.WSL.SAMPLING.SAMPLING_TYPE = "high"
    _C.WSL.SAMPLING.LB = 100
    _C.WSL.SAMPLING.LAMBDA_MAX = 0.51
    ...
    _C.OICRPLUS = CN()
    _C.OICRPLUS.BBOX_UPDATE = False
    _C.OICRPLUS.PROPOSAL_NUM = 100000


    # lr for refinement branch
    _C.SOLVER.REFINE_LR_SCALE = 1.0
    _C.SOLVER.REFINE_SCALE_ON = False
    _C.SOLVER.AMP = False

    _C.WSODEVAL = CN()
    _C.WSODEVAL.SAVE_DETECTION_RESULT = False
    _C.WSODEVAL.SAVE_PATH = ""